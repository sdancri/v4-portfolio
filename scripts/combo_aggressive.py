"""Combo backtest cu SL=3% AGGRESSIVE pe toate perechile.

TP-uri din best Return @ SL=3% per pereche (din sweeps individuale):
  MNT  : SL=3%, no TP
  AXS  : SL=3%, no TP
  ILV  : SL=3%, TP=12%  (best return: hull=8/kj=36/snkb=40)
  AERO : SL=3%, TP=20%  (best return)
"""
...
# Aggressive SL=3% — TP-uri din best Return @ SL=3% (sweeps)
AGGR = {
    "MNTUSDT":  {"hull": 8,  "kj": 48, "snkb": 40, "sl": 0.03, "tp": None,
                 "step": 0.1, "start": "2023-10-01"},
    "AXSUSDT":  {"hull": 10, "kj": 48, "snkb": 26, "sl": 0.03, "tp": None,
                 "step": 0.1, "start": "2023-10-01"},
    "ILVUSDT":  {"hull": 8,  "kj": 36, "snkb": 40, "sl": 0.03, "tp": 0.12,
                 "step": 0.01, "start": "2023-10-01"},
    "AEROUSDT": {"hull": 10, "kj": 60, "snkb": 26, "sl": 0.03, "tp": 0.20,
...
def run_combo(symbols: tuple[str, ...]) -> dict:
    qty_steps = {s: AGGR[s]["step"] for s in symbols}
    start = max(pd.Timestamp(AGGR[s]["start"], tz="UTC") for s in symbols)
    end = pd.Timestamp("2026-04-25", tz="UTC")
    months = (end - start).days / 30.4

    combo_dir = Path(f"/tmp/aggr_{'_'.join(s.replace('USDT','') for s in symbols)}")
    ...
    cfg = AppConfig(
        portfolio=PortfolioConfig(name="aggr", pool_total=100.0, leverage=15,
                                  cap_pct_of_max=0.95, taker_fee=0.00055, slippage_bps=0.0),
        pairs=[make_pair(s, AGGR[s]) for s in symbols],
...
print("\n" + "="*100)
print("AGGRESSIVE COMBOS — SL=3% pe TOATE perechile (risk_per_trade=7%, lev=20×)")
print("Settings: MNT(noTP), AXS(noTP), ILV(TP=12%), AERO(TP=20%)")
print("Position size = $7 / 0.03 = $233 = 2.33× equity per trade!")
print("="*100)
